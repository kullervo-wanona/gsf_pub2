...
for sample_id, image_ind in enumerate(image_ind_list):
    ...
    # real_image_np = helper.load_image(real_image_path, size=None)
    row_list = []
    for layer_id in list(range(n_layers)):
    # for layer_id in list(range(n_layers))[::-1]:
        if layer_id % skip_layers != 0: continue
        curr_layer_dir = main_dir+'inverse_layers/inverse_layer'+'_'+ str(layer_id) +'_test/'
        image_path = curr_layer_dir+image_name
        print(image_path)
        image_np = helper.load_image(image_path, size=None)
        row_list.append(image_np[np.newaxis])
...
helper.visualize_image_matrix(raster, block_size=None, max_rows=None, padding=[4, 4], save_path_list=['./inverse_blah.png'], verbosity_level=4)
