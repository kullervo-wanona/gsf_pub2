...
class DataLoader:
    def __init__(self, batch_size):
        self.dataset_name = 'CelebA 178'
        self.batch_size = batch_size
        self.buffer_batch_ratio = 100
        self.iter = 0
        self.image_size = [-1, 178, 178, 3]
        self.dataset_dir = str(Path.home())+'/Datasets/CelebA/CelebA178/'
        try: self.load_h5()
    ...
    def load_h5(self):
        processed_file_path = self.dataset_dir + 'celebA178.h5'
    ...
    def create_h5(self, chunk_size=10000):
        ...
        for i, file in enumerate(zipfile_object.infolist()):
            if i > 0:
                ...
                if training_mask[i-1]:
                    training_images_data[training_index, ...] = cropped_image
                    training_index += 1
                elif validation_mask[i-1]:
                    validation_images_data[validation_index, ...] = cropped_image
                    validation_index += 1
                elif test_mask[i-1]:
                    test_images_data[test_index, ...] = cropped_image
                    test_index += 1
            else:
                zipfile_object.extract(file, source_dir)
