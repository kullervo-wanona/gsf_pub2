...
class DataLoader:
    def __init__(self, batch_size, validation_split=True):
        self.dataset_name = 'MNIST'
        self.batch_size = batch_size
        self.iter = 0
        self.image_size = [-1, 1, 28, 28]
        self.dataset_dir = str(Path.home())+'/Datasets/MNIST/MNIST/'
    ...
    def load_h5(self):
        if self.validation_split: processed_file_path = self.dataset_dir + 'mnist_with_val.h5'
        else: processed_file_path = self.dataset_dir + 'mnist.h5'
    ...
    def create_h5(self):
        ...
        training_images = mnist_train_images.numpy().reshape(self.image_size)
        if self.validation_split:
            validation_images = mnist_validation_images.numpy().reshape(self.image_size)
        test_images = mnist_test_images.numpy().reshape(self.image_size)
