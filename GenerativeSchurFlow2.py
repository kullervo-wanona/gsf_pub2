...
class GenerativeSchurFlow(torch.nn.Module):
    def __init__(self, c_in, n_in, k_list, squeeze_list, final_actnorm=False):
        ...
        print('**********************************************************\n')
        actnorm_layers, pre_affine_layers, pre_nonlin_layers, conv_nonlin_layers = [], [], [], [] 
        conv_layers, post_nonlin_layers, post_affine_layers = [], [], []
    ...
    def transform_with_logdet(self, x, initialization=False):
        actnorm_logdets, pre_affine_logdets, pre_nonlin_logdets = [], [], []
        conv_logdets, conv_nonlin_logdets, post_nonlin_logdets, post_affine_logdets = [], [], [], []
        ...
        for layer_id, k in enumerate(self.k_list): 
            ...
            curr_y, pre_affine_logdet = self.pre_affine_layers[layer_id].transform_with_logdet(curr_y)
        ...
        y = curr_y
        total_logdet = sum(actnorm_logdets)+sum(pre_affine_logdets)+sum(pre_nonlin_logdets)+\
                        sum(conv_logdets)+sum(conv_nonlin_logdets)+sum(post_nonlin_logdets)+sum(post_affine_logdets)
    ...
    def inverse_transform(self, y):
        with torch.no_grad():
            ...
            for layer_id in range(len(self.k_list)-1, -1,-1):
                ...
                curr_y = self.pre_affine_layers[layer_id].inverse_transform(curr_y)
